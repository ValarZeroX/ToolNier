'use client';
import React, { useState } from 'react';
import { Container, Grid, Paper, Text, Title, Tooltip, TableOfContents } from '@mantine/core';
import { useClipboard } from '@mantine/hooks';
import { VirtuosoGrid } from 'react-virtuoso';
import { useTranslation } from "../../../i18n/client";


type SymbolClientProps = {
    lng: string;
};

const symbolCategories = [
    {
        category: '特殊符號',
        symbols: ['ร', 'ಗ', 'ย', 'ஃ', '⁜', '❡', '๑', '▧', '◘', '▩', '◙', '▨', 'ಔ', '☎', '☏', '⍦', '▤', '▥', '▦', '✠', '＠', '۝', 'ಓ', 'ↂ', 'ూ', 'ி', 'ౖ', 'ి', 'ృ', 'ꦿ', '᭄', 'ﺴ', '✈', '✉', '✁', '✎', '✐', '〄', '＃', '‡', '⌚', '☢', '▪', '▫', '✆', '✑', '✒', '✏', '⌨', '✂', '✃', '✄', '◧', '◨', 'ბ', 'ფ', '⌥', '⎋', 'ᴴᴰ', 'Π', '☣', 'ჯ', 'ಌ', '௰', '❏', '❐', '❒', '⌘', '⌛', '೪', '☠', 'з', 'ஜ', 'ஒ', 'ண', 'ѣ', 'Ѥ', 'Ѧ', 'Ѩ', 'Ѫ', 'Ѭ', 'Ѯ', 'ѯ', 'Ѱ', 'ѱ', 'Ѳ', 'Ѷ', 'Ѻ', 'Ѽ', 'Ѿ', 'ѿ', 'Ҁ', '҈', '҉', 'Ҋ', 'Ҍ', 'Ҏ', 'Қ', 'Җ', 'Ҕ', 'Ҙ', '曱', '甴', '囍', 'இ', 'ஆ', '௫', 'ஐ', 'ﻬ', '囸', '凸', '⌾', '⍟', '⌽', '⍉', '⍧', '⍪', '⍫', '⍬', '⍭', '⌿', '⍀', '⍊', '⍑', '⍋', '⍒', '⍎', '⍕', '⍱', '⍲', '⍰', '⌹', '⌺', '⌻', '⌼', '⍞', '⚰', '⚱', '⚫', '⚪', '☕', '⚽', '⚾', '⚓', '⚔', '⏍', '⎅', '⏛', '⍁', '⍂', '⍠', '▢', '⎚', '⧉', '⟥', '⟤', '⚖', '⚈', '⚉', '⚐', '⚑']
    },
    {
        category: '三角形符號',
        symbols: ['△', '▽', '▲', '▼', '►', '◄', '▸', '◂', '▴', '▾', '▵', '▿', '◁', '▷', '▹', '◃', '◀', '▶', '◢', '◣', '◥', '◤', '◬', '⟁', '⧋', '⧊', '⦉', '⦊', '⧩', '⧨', '◭', '◮', '◅', '▻', '∆', '∇', '𓇮', '⫷', '⫸']
    },
    {
        category: '鑽石形符號',
        symbols: ['◇', '◆', '◈', '◊', '⟐', '⋄', '⟡', '⎏', '⎐', '⎑', '⎒', '⟢', '⟣', '⧪', '⧫', '⧰', '⍚', '⟠', '⟕', '⟖', '⟗']
    },
    {
        category: '圓形符號',
        symbols: ['⊖', '⊘', '⊚', '⊛', '⊜', '⊝', '◉', '○', '◌', '⧂', '◍', '◎', '●', '◐', '◑', '◒', '◓', '◔', '◕', '⧃', '◖', '◗', '◦', '◯', '⦾', '⦿', '⊕', '⊗', '⛒', '⨷', '⬲', '❍', '⬤', '⬬', '⬭', '⬮', '⬯', '⬰', '⧭', '⧬', '⧳', '⧲', 'ಠ', 'ಥ', 'ຈ', 'Ꙩ', 'ꙩ', '⚆', '⚇', '𓏗', '𓂀', '𓁿', '𓁾', '𓁽', '𓁼', '𓁻', '𓁺', '𓁹', '𓂉', '𓂊', '𓎲', '𓎱']
    },
    {
        category: '箭頭符號',
        symbols: ['»', '«', '›', '‹', '≫', '≪', '↑', '↓', '←', '→', '↖', '↗', '↙', '↘', '↔', '↕', '⍅', '⍆', '⍏', '⍖', '⇝', '⇜', '➼', '➽', '➸', '➳', '➺', '➻', '➴', '➵', '➶', '➷', '➹', '➩', '➪', '➫', '➬', '➭', '➮', '➯', '➱', '➲', '➾', '➔', '➘', '➙', '➚', '➛', '➜', '➝', '➞', '➟', '➠', '➡', '➢', '➣', '➤', '➥', '➦', '➧', '➨', '↚', '↛', '↜', '↝', '↞', '↟', '↠', '↡', '↢', '↣', '↤', '↥', '↦', '↧', '↨', '⇄', '⇅', '⇵', '⇆', '⇇', '⇈', '⇉', '⇊', '⇋', '⇌', '⇍', '⇎', '⇏', '⇐', '⇑', '⇒', '⇓', '⇔', '⇖', '⇗', '⇘', '⇙', '⥅', '⥆', '↩', '↪', '↫', '↬', '↭', '↮', '↯', '↰', '↱', '↲', '↳', '↴', '↵', '↶', '↷', '↸', '↹', '↺', '↻', '↼', '↽', '↾', '↿', '⇀', '⇁', '⇂', '⇃', '⇞', '⇟', '⇠', '⇡', '⇢', '⇣', '⇤', '⇥', '⇦', '⇧', '⇨', '⇩', '⇪', '⌦', '⌫', '⬀', '⬁', '⬂', '⬃', '⬄', '⬅', '⬆', '⬇', '⬈', '⬉', '⬊', '⬋', '⬌', '⬍', '⇫', '⇬', '⇶', '⬱', '⬳', '⬸', '⬿', '⏎', '⌂', '⥒', '⥓', '⥔', '⥕', '⥖', '⥗', '⥘', '⥙', '⤞', '⤝', '⤠', '⤟', '⥂', '⥃', '⥄', '⤣', '⤤', '⤥', '⤦', '⤡', '⤢', '⤧', '⤨', '⤩', '⤪', '⤭', '⤮', '⤱', '⤲', '⤯', '⤰', '⤻', '⤺', '⤼', '⤽', '⤹', '⤸', '⤿', '⤾', '⤊', '⤋', '⟰', '⟱', '⭅', '⭆', '⟻', '⟼', '⟸', '⟹', '⟺', '⟽', '⟾', '⥼', '⥽', '⥾', '⥿', '⤌', '⤍', '⤎', '⤏', '⤐', '⤑', '⮰', '⮱', '⮲', '⮳', '⮴', '⮵', '⮶', '⮷', '⮨', '⮩', '⮫', '⮬', '⮭', '⮮', '⮯', '⍃', '⍄', '⍇', '⍈', '⍐', '⍗', '⍌', '⍓', '⍍', '⍔', '⥇', '⥈', '⥉', '⥊', '⥋', '⥌', '⥍', '⥎', '⥐', '⥏', '⥑', '⥚', '⥛', '⥜', '⥝', '⥞', '⥟', '⥠', '⥡', '⥢', '⥤', '⥣', '⥥', '⥦', '⥧', '⥨', '⥩', '⥪', '⥫', '⥬', '⥭', '⥮', '⥯', '⥱', '⥲', '⥳', '⥴', '⥵', '⥶', '⥸', '⥹', '⥻', '⥷', '⥺', '⟴', '⇭', '⇮', '⇯', '⇰', '⇱', '⇲', '⇳', '⇴', '⇷', '⇸', '⇹', '⇺', '⇻', '⇼', '⇽', '⇾', '⇿', '⤳', '⤴', '⤵', '⤶', '⤷', '⥰', '⥀', '⥁', '⌅', '⌆', '⌤']
    },
    {
        category: '錯對符號',
        symbols: ['☑', '✓', '✔', '⎷', '⍻', '☒', '⌧', '✇', '✖', '✗', '✘', '✕', '⤫', '⤬', '𒉽', '☐']
    },
    {
        category: '愛心符號',
        symbols: ['❤', '♥', '♡', '❥', '❣', '❦', '❧', '☙', '۵', 'დ', 'ღ', 'ო', 'Ꮼ']
    },
    {
        category: '環保符號',
        symbols: ['♲', '♻', '♽', '♼', '♺', '♳', '♴', '♵', '♶', '♷', '♸', '♹']
    },
    {
        category: '十二星座',
        symbols: ['♈', '♉', '♊', '♋', '♌', '♍', '♎', '♏', '♐', '♑', '♒', '♓']
    },
    {
        category: '十二生肖',
        symbols: ['🐭', '🐮', '🐯', '🐰', '🐲', '🐍', '🐴', '🐑', '🐵', '🐔', '🐶', '🐷']
    },
    {
        category: '數字符號',
        symbols: ['🄀', '⒈', '⒉', '⒊', '⒋', '⒌', '⒍', '⒎', '⒏', '⒐', '⒑', '⒒', '⒓', '⒔', '⒕', '⒖', '⒗', '⒘', '⒙', '⒚', '⒛', '⓪', '①', '②', '③', '④', '⑤', '⑥', '⑦', '⑧', '⑨', '⑩', '⑪', '⑫', '⑬', '⑭', '⑮', '⑯', '⑰', '⑱', '⑲', '⑳', '㉑', '㉒', '㉓', '㉔', '㉕', '㉖', '㉗', '㉘', '㉙', '㉚', '㉛', '㉜', '㉝', '㉞', '㉟', '㊱', '㊲', '㊳', '㊴', '㊵', '㊶', '㊷', '㊸', '㊹', '㊺', '㊻', '㊼', '㊽', '㊾', '㊿', '➀', '➁', '➂', '➃', '➄', '➅', '➆', '➇', '➈', '➉', '⓿', '❶', '❷', '❸', '❹', '❺', '❻', '❼', '❽', '❾', '❿', '⓫', '⓬', '⓭', '⓮', '⓯', '⓰', '⓱', '⓲', '⓳', '⓴', '⓵', '⓶', '⓷', '⓸', '⓹', '⓺', '⓻', '⓼', '⓽', '⓾', '⑴', '⑵', '⑶', '⑷', '⑸', '⑹', '⑺', '⑻', '⑼', '⑽', '⑾', '⑿', '⒀', '⒁', '⒂', '⒃', '⒄', '⒅', '⒆', '⒇', '㈠', '㈡', '㈢', '㈣', '㈤', '㈥', '㈦', '㈧', '㈨', '㈩', '㊀', '㊁', '㊂', '㊃', '㊄', '㊅', '㊆', '㊇', '㊈', '㊉', '１', '２', '３', '４', '５', '６', '７', '８', '９', '０', '𝟭', '𝟮', '𝟯', '𝟰', '𝟱', '𝟲', '𝟳', '𝟴', '𝟵', '𝟬', '𝟙', '𝟚', '𝟛', '𝟜', '𝟝', '𝟞', '𝟟', '𝟠', '𝟡', '𝟘', '१', '२', '३', '४', '५', '६', '७', '८', '९', '०', 'Ⅰ', 'Ⅱ', 'Ⅲ', 'Ⅳ', 'Ⅴ', 'Ⅵ', 'Ⅶ', 'Ⅷ', 'Ⅸ', 'Ⅹ', 'Ⅺ', 'Ⅻ', 'ⅰ', 'ⅱ', 'ⅲ', 'ⅳ', 'ⅴ', 'ⅵ', 'ⅶ', 'ⅷ', 'ⅸ', 'ⅹ', 'ⅺ', 'ⅻ', '㉈', '㉉', '㉊', '㉋', '㉌', '㉍', '㉎', '㉏', '㆒', '㆓', '㆔', '㆕', '⁰', '¹', '²', '³', '⁴', '⁵', '⁶', '⁷', '⁸', '⁹', '₀', '₁', '₂', '₃', '₄', '₅', '₆', '₇', '₈', '₉']
    },
    {
        category: '蘇州碼子',
        symbols: ['〇', '〡', '〢', '〣', '〤', '〥', '〦', '〧', '〨', '〩', '〸', '〹', '卄', '〺', '卅', '卌']
    },
    {
        category: '英文符號',
        symbols: ['ⓐ', 'ⓑ', 'ⓒ', 'ⓓ', 'ⓔ', 'ⓕ', 'ⓖ', 'ⓗ', 'ⓘ', 'ⓙ', 'ⓚ', 'ⓛ', 'ⓜ', 'ⓝ', 'ⓞ', 'ⓟ', 'ⓠ', 'ⓡ', 'ⓢ', 'ⓣ', 'ⓤ', 'ⓥ', 'ⓦ', 'ⓧ', 'ⓨ', 'ⓩ', 'Ⓐ', 'Ⓑ', 'Ⓒ', 'Ⓓ', 'Ⓔ', 'Ⓕ', 'Ⓖ', 'Ⓗ', 'Ⓘ', 'Ⓙ', 'Ⓚ', 'Ⓛ', 'Ⓜ', 'Ⓝ', 'Ⓞ', 'Ⓟ', 'Ⓠ', 'Ⓡ', 'Ⓢ', 'Ⓣ', 'Ⓤ', 'Ⓥ', 'Ⓦ', 'Ⓧ', 'Ⓨ', 'Ⓩ', '⒜', '⒝', '⒞', '⒟', '⒠', '⒡', '⒢', '⒣', '⒤', '⒥', '⒦', '⒧', '⒨', '⒩', '⒪', '⒫', '⒬', '⒭', '⒮', '⒯', '⒰', '⒱', '⒲', '⒳', '⒴', '⒵', '🄐', '🄑', '🄒', '🄓', '🄔', '🄕', '🄖', '🄗', '🄘', '🄙', '🄚', '🄛', '🄜', '🄝', '🄞', '🄟', '🄠', '🄡', '🄢', '🄣', '🄤', '🄥', '🄦', '🄧', '🄨', '🄩', '🄰', '🄱', '🄲', '🄳', '🄴', '🄵', '🄶', '🄷', '🄸', '🄹', '🄺', '🄻', '🄼', '🄽', '🄾', '🄿', '🅀', '🅁', '🅂', '🅃', '🅄', '🅅', '🅆', '🅇', '🅈', '🅉', '🅰', '🅱', '🅲', '🅳', '🅴', '🅵', '🅶', '🅷', '🅸', '🅹', '🅺', '🅻', '🅼', '🅽', '🅾', '🅿', '🆀', '🆁', '🆂', '🆃', '🆄', '🆅', '🆆', '🆇', '🆈', '🆉', '🅐', '🅑', '🅒', '🅓', '🅔', '🅕', '🅖', '🅗', '🅘', '🅙', '🅚', '🅛', '🅜', '🅝', '🅞', '🅟', '🅠', '🅡', '🅢', '🅣', '🅤', '🅥', '🅦', '🅧', '🅨', '🅩', '𝐀', '𝐁', '𝐂', '𝐃', '𝐄', '𝐅', '𝐆', '𝐇', '𝐈', '𝐉', '𝐊', '𝐋', '𝐌', '𝐍', '𝐎', '𝐏', '𝐐', '𝐑', '𝐒', '𝐓', '𝐔', '𝐕', '𝐖', '𝐗', '𝐘', '𝐙', '𝐚', '𝐛', '𝐜', '𝐝', '𝐞', '𝐟', '𝐠', '𝐡', '𝐢', '𝐣', '𝐤', '𝐥', '𝐦', '𝐧', '𝐨', '𝐩', '𝐪', '𝐫', '𝐬', '𝐭', '𝐮', '𝐯', '𝐰', '𝐱', '𝐲', '𝐳', '𝘈', '𝘉', '𝘊', '𝘋', '𝘌', '𝘍', '𝘎', '𝘏', '𝘐', '𝘑', '𝘒', '𝘓', '𝘔', '𝘕', '𝘖', '𝘗', '𝘘', '𝘙', '𝘚', '𝘛', '𝘜', '𝘝', '𝘞', '𝘟', '𝘠', '𝘡', '𝘢', '𝘣', '𝘤', '𝘥', '𝘦', '𝘧', '𝘨', '𝘩', '𝘪', '𝘫', '𝘬', '𝘭', '𝘮', '𝘯', '𝘰', '𝘱', '𝘲', '𝘳', '𝘴', '𝘵', '𝘶', '𝘷', '𝘸', '𝘹', '𝘺', '𝘻', '𝘼', '𝘽', '𝘾', '𝘿', '𝙀', '𝙁', '𝙂', '𝙃', '𝙄', '𝙅', '𝙆', '𝙇', '𝙈', '𝙉', '𝙊', '𝙋', '𝙌', '𝙍', '𝙎', '𝙏', '𝙐', '𝙑', '𝙒', '𝙓', '𝙔', '𝙕', '𝙖', '𝙗', '𝙘', '𝙙', '𝙚', '𝙛', '𝙜', '𝙝', '𝙞', '𝙟', '𝙠', '𝙡', '𝙢', '𝙣', '𝙤', '𝙥', '𝙦', '𝙧', '𝙨', '𝙩', '𝙪', '𝙫', '𝙬', '𝙭', '𝙮', '𝙯', '𝓐', '𝓑', '𝓒', '𝓓', '𝓔', '𝓕', '𝓖', '𝓗', '𝓘', '𝓙', '𝓚', '𝓛', '𝓜', '𝓝', '𝓞', '𝓟', '𝓠', '𝓡', '𝓢', '𝓣', '𝓤', '𝓥', '𝓦', '𝓧', '𝓨', '𝓩', '𝓪', '𝓫', '𝓬', '𝓭', '𝓮', '𝓯', '𝓰', '𝓱', '𝓲', '𝓳', '𝓴', '𝓵', '𝓶', '𝓷', '𝓸', '𝓹', '𝓺', '𝓻', '𝓼', '𝓽', '𝓾', '𝓿', '𝔀', '𝔁', '𝔂', '𝔃', '𝒜', 'ℬ', '𝒞', '𝒟', 'ℰ', 'ℱ', '𝒢', 'ℋ', 'ℐ', '𝒥', '𝒦', 'ℒ', 'ℳ', '𝒩', '𝒪', '𝒫', '𝒬', 'ℛ', '𝒮', '𝒯', '𝒰', '𝒱', '𝒲', '𝒳', '𝒴', '𝒵', '𝒶', '𝒷', '𝒸', '𝒹', '𝑒', '𝒻', '𝑔', '𝒽', '𝒾', '𝒿', '𝓀', '𝓁', '𝓂', '𝓃', '𝑜', '𝓅', '𝓆', '𝓇', '𝓈', '𝓉', '𝓊', '𝓋', '𝓌', '𝓍', '𝓎', '𝓏', '𝔸', '𝔹', 'ℂ', '𝔻', '𝔼', '𝔽', '𝔾', 'ℍ', '𝕀', '𝕁', '𝕂', '𝕃', '𝕄', 'ℕ', '𝕆', 'ℙ', 'ℚ', 'ℝ', '𝕊', '𝕋', '𝕌', '𝕍', '𝕎', '𝕏', '𝕐', 'ℤ', '𝕒', '𝕓', '𝕔', '𝕕', '𝕖', '𝕗', '𝕘', '𝕙', '𝕚', '𝕛', '𝕜', '𝕝', '𝕞', '𝕟', '𝕠', '𝕡', '𝕢', '𝕣', '𝕤', '𝕥', '𝕦', '𝕧', '𝕨', '𝕩', '𝕪', '𝕫', '𝕬', '𝕭', '𝕮', '𝕯', '𝕰', '𝕱', '𝕲', '𝕳', '𝕴', '𝕵', '𝕶', '𝕷', '𝕸', '𝕹', '𝕺', '𝕻', '𝕼', '𝕽', '𝕾', '𝕿', '𝖀', '𝖁', '𝖂', '𝖃', '𝖄', '𝖅', '𝖆', '𝖇', '𝖈', '𝖉', '𝖊', '𝖋', '𝖌', '𝖍', '𝖎', '𝖏', '𝖐', '𝖑', '𝖒', '𝖓', '𝖔', '𝖕', '𝖖', '𝖗', '𝖘', '𝖙', '𝖚', '𝖛', '𝖜', '𝖝', '𝖞', '𝖟', '𝔄', '𝔅', 'ℭ', '𝔇', '𝔈', '𝔉', '𝔊', 'ℌ', 'ℑ', '𝔍', '𝔎', '𝔏', '𝔐', '𝔑', '𝔒', '𝔓', '𝔔', 'ℜ', '𝔖', '𝔗', '𝔘', '𝔙', '𝔚', '𝔛', '𝔜', 'ℨ', '𝔞', '𝔟', '𝔠', '𝔡', '𝔢', '𝔣', '𝔤', '𝔥', '𝔦', '𝔧', '𝔨', '𝔩', '𝔪', '𝔫', '𝔬', '𝔭', '𝔮', '𝔯', '𝔰', '𝔱', '𝔲', '𝔳', '𝔴', '𝔵', '𝔶', '𝔷', 'ᴬ', 'ᴮ', 'ᴰ', 'ᴱ', 'ᴳ', 'ᴴ', 'ᴵ', 'ᴶ', 'ᴷ', 'ᴸ', 'ᴹ', 'ᴺ', 'ᴼ', 'ᴾ', 'ᴿ', 'ᵀ', 'ᵁ', 'ᵂ', 'ᵡ', 'ᵞ', 'ᵃ', 'ᵇ', 'ᶜ', 'ᵈ', 'ᵉ', 'ᶠ', 'ᵍ', 'ʰ', 'ʲ', 'ᵏ', 'ˡ', 'ᵐ', 'ᵒ', 'ᵖ', 'ʳ', 'ˢ', 'ᵗ', 'ᵘ', 'ᵛ', 'ʷ', 'ˣ', 'ʸ', 'ᶻ', 'ₕ', 'ᵢ', 'ⱼ', 'ₖ', 'ₗ', 'ₘ', 'ₙ', 'ₚ', 'ᵣ', 'ₛ', 'ₜ', 'ᵤ', 'ᵥ', 'ᵪ', 'ᶥ', 'ᶿ']
    },
    {
        category: '角度符號',
        symbols: ['⦯', '⦮', '⦨', '⦩', '⦬', '⦭', '⦪', '⦫', '⦥', '⦤', '⦦', '⦧', '⦛', '⦡', '⦠', '⦜', '⦟', '⦣', '⦢', '⦞', '⦝']
    },
    {
        category: '數學符號',
        symbols: ['⁺', '⁻', '⁼', '⁽', '⁾', '₊', '₋', '₌', '₍', '₎', '≒', '≌', '∵', '∴', '⛬', '⛼', '⛻', '×', '／', '⧾', '⧿', '﹣', '±', '≦', '≧', '﹤', '﹥', '≠', '≡', '╳', '⊾', '∷', '÷', 'ˇ', '∫', '∮', '∠', '∟', '⊿', '＋', '﹢', '⊥', '⅟', '½', '↉', '⅓', '⅔', '¼', '¾', '⅕', '⅖', '⅗', '⅘', '⅙', '⅚', '⅐', '⅛', '⅜', '⅝', '⅞', '⅑', '⅒', '∬', '∭', '∯', '∰', '∱', '∲', '∳', '≂', '≃', '≄', '≅', '≆', '≇', '≈', '≉', '≊', '≋', '≍', '≎', '≏', '≐', '≑', '≓', '≔', '≕', '≖', '≗', '≘', '≙', '≚', '≛', '≜', '≝', '≞', '≟', '≣', '≤', '≥', '≨', '≩', '⊰', '⊱', '⋛', '⋚', 'ⁱ', 'ⁿ', 'ₐ', 'ₑ', 'ₒ', 'ₓ', 'ₔ', '∑', 'π', '‱', '‰', '∶', '∸', '∹', '∺', '∻', '∼', '∽', '∾', '∿', '≀', '‖', '∧', '∨', '|a|', '∩', '∪', '∈', '∋', '⋙', '⋘', '⋟', '⋞', '⋣', '⋢', '⋥', '⋤', '⋩', '⋨', '⋧', '⋦', '⋫', '⋪', '⋭', '⋬', '⋰', '⋱', '⋮', '⋯', '∝', '∞', '∡', '∢', '∀', '∁', '∂', '∃', '∄', '∅', '√', '∛', '∜']
    },
    {
        category: '單位符號',
        symbols: ['㏑', '㏒', '㏎', '㏕', '㎎', '㎏', '㎜', '㎝', '㎞', '㏄', '§', '№', 'ℂ', '℄', 'ℇ', '℈', '㎙', '㎚', '㎛', '㎟', '㎠', '㎡', '㎢', '㎣', '㎤', '㎥', '㎦', '㎅', '㎆', '㎇', '㎕', '㎖', '㎗', '㎘', '㎍', '㏀', '㏁', '㍱', '㍲', '㍳', '㍴', '㍵', '㍶', '㎀', '㎁', '㎂', '㎃', '㎄', '㎈', '㎉', '㎊', '㎋', '㎌', '㎐', '㎑', '㎒', '㎓', '㎔', '㏃', '㏅', '㏆', '㏇', '㏈', '㏉', '㏊', '㏋', '㏌', '㏍', '㏏', '㏐', '㏓', '℔', '㏔', '㏖', '㏗', '㏙', '㏛', '㏜', '㏝', '㏚', '℡', '℠', '™', '℀', '℁', '℅', '℆', '©', '🄯', '®', '℗']
    },
    {
        category: '框線符號',
        symbols: ['┬', '┴', '├', '─', '┼', '┤', '┌', '┐', '╞', '═', '╪', '╡', '│', '▕', '└', '┘', '╭', '╮', '╰', '╯', '╔', '╦', '╗', '╠', '═', '╬', '╣', '╓', '╥', '╖', '╒', '╤', '╕', '║', '╚', '╩', '╝', '╟', '╫', '╢', '╙', '╨', '╜', '╞', '╪', '╡', '╘', '╧', '╛', 'ᐟ', '‒', '―', '⁃', '⋐', '⋑', '⋒', '⋓', '⌒', '⌜', '⌝', '⌞', '⌟', '⎯', '━', '┄', '┅', '┆', '┇', '┈', '┉', '┊', '┋', '﹉', '﹊', '﹍', '﹍', '┎', '┓', '┕', '┛', '╊', '╌', '╍', '╱', '╲', '╴', '╵', '╶', '╷', '╸', '╹', '╺', '╻', '╼', '╽', '╾', '╿', '◜', '◝', '◞', '◟', '◠', '◡', '⌈', '⌊', '—', '⌉', '⌋', '☡', '༘', '෴', '؄', 'ᔐ', 'ᔑ', 'ᔓ', 'ᔕ', '꧖', '⦚', '⌇', '𓂃', '𓂅', '𓏲', '𓍱', '𓍲', '𓎆', '⎛', '⎞', '⎝', '⎠']
    },
    {
        category: '填色方塊',
        symbols: ['＿', 'ˍ', '▁', '▂', '▃', '▄', '▅', '▆', '▇', '█', '▏', '▎', '▍', '▌', '▐', '▋', '▊', '▉', '◪', '◫', '▓', '░', '▒', '❑', '❒', '❖', '❚', '⊞', '⊟', '⊠', '⊡', '⎔', '▀', '▯', '▮', '▰', '▱', '▬', '□', '■', '◩', '⚿', '⛝', '❎', '⟎', '⟏', '⧄', '⧅', '⧆', '⧇', '⧈', '▣', '⧯', '⧮', '▖', '▗', '▘', '▝', '▚', '▞', '▙', '▟', '▛', '▜']
    },
    {
        category: '手勢符號',
        symbols: ['⍝', '☜', '☞', '☚', '☛', '☟', '☝', '✍', '✋', '✌', '✊']
    },
    {
        category: '翅膀符號',
        symbols: ['༺', '༻', '꧁', '꧂', '𓆩', '𓆪', 'ʚ', 'ɞ', 'ε', 'з', '⊱', '⊰', '⋛', '⋚', '๑', 'ɷ', 'ᦂ', 'ᦦ', 'ઈ', 'ઉ', 'ཉིཾ', 'ཉྀ', '⋋', '⋌', '꒰', '꒱', '⚞', '⚟', '≥', '≤', '⋙', '⋘', '⋩', '⋨', '∋', '∈', '⋟', '⋞']
    },
    {
        category: '圖騰符號',
        symbols: ['𓊈', '𓊉', '𓊆', '𓊇', '⩨', '⩩', '༚', '༛', '༜', '༞', '༟', '࿎', '𒀺', '𒀹', '𒍻', '𒑠', '𒁹', '𒀸', '𒅂', '𒁂', '𒇹', '𒌋', '𒑱', '𒌍', '𒉼', '𒈱', '𒈝', '𒈞', '𒐆', '𒆳', '𒍞', '𒑴', '𒆙', '𒆜', '𒉺', '𒌑', '𒀽', '𒆸', '𒇻', '𒁈', '𒇔', '𒐦', '𒐤', '𒐪', '𒐬', '𒐫', '𒐮', '𒐰', '𒀱', '𒅒', '𒋲', '𒇫', '𒍟', '𒅄', 'ဪ', '꧅']
    },
    {
        category: '表意文字組合符',
        symbols: ['⿰', '⿱', '⿲', '⿳', '⿴', '⿵', '⿶', '⿷', '⿸', '⿹', '⿺', '⿻', '〾']
    },
    {
        category: '天氣符號',
        symbols: ['☼', '☀', '⛭', '⛮', '⛯', '☁', '☂', '☄', '☾', '☽', '❄', '☃', '☈', '⊙', '☉', '°', '℃', '℉', '❅', '✺', 'ϟ', '☇', '♨', '🜚', '🜨', '♃', '♄', '♅', '⛢', '♆', '⚳', '⚴', '⚵', '⚶', '⚕', '⚷', '♁', '⧬', '♇', '☌', '⚺', '⚻', '∠', '⚹', 'Q', '□', '△', '⚼', '☍', '☊', '☋', '⚸', '℞', '⊗', '🌑', '🌒', '🌓', '🌔', '🌕', '🌖', '🌗', '🌘']
    },
    {
        category: '將棋符號',
        symbols: ['☖', '⛉', '☗', '⛊']
    },
    {
        category: '西洋棋符號',
        symbols: ['♔', '♕', '♖', '♗', '♘', '♙', '♚', '♛', '♜', '♝', '♞', '♟']
    },
    {
        category: '象棋符號',
        symbols: ['🩠', '🩡', '🩢', '🩣', '🩤', '🩥', '🩦', '🩧', '🩨', '🩩', '🩪', '🩫', '🩬', '🩭']
    },
    {
        category: '宗教圖案',
        symbols: ['†', '☨', '✞', '✝', '☥', '☦', '☓', '☩', '☯', '☧', '☬', '☫', '☸', '♁', '✙', '♆', '✚', '✛', '✜', '✟', '卍', '卐', '☮', '☤', '☭', '♰', '♱', '⚚', '۞', '࿊', '۩', '࿅', '࿉', '𓉴', '𓉸', '﷽', '﷽', 'ૐ']
    },
    {
        category: '星星符號',
        symbols: ['★', '✰', '☆', '✩', '✫', '✬', '✭', '✮', '✡', '⍣', '⋆', '☪', '✢', '✣', '✤', '✥', '✦', '✧', '✪', '✯', '❂', '❉', '✱', '✲', '✳', '✴', '✵', '✶', '✷', '✸', '✹', '❊', '✻', '✼', '❆', '❇', '❈', '⁂', '⁑', '⁎', '*', '⁕', '𒀭', '𒄬', '𒀯', '𓇼', '𓇻', '⚝', '⛤', '⛥', '⛦', '⛧', '𓇽', '꙳', '⭑', '⭒']
    },
    {
        category: '貨幣符號',
        symbols: ['￥', '￡', '€', '￠', '¥', '₮', '৲', '৳', '௹', '฿', '៛', '₠', '₡', '₢', '₣', '₤', '₥', '₦', '₧', '₨', '₩', '₪', '₫', '₭', '₯', '₰', '₱', '₲', '₳', '₴', '₵', '﷼', '$', '¤', 'ƒ', '〒']
    },
    {
        category: '時間日期',
        symbols: ['㏂', '㏘', '㋀', '㋁', '㋂', '㋃', '㋄', '㋅', '㋆', '㋇', '㋈', '㋉', '㋊', '㋋', '㏠', '㏡', '㏢', '㏣', '㏤', '㏥', '㏦', '㏧', '㏨', '㏩', '㏪', '㏫', '㏬', '㏭', '㏮', '㏯', '㏰', '㏱', '㏲', '㏳', '㏴', '㏵', '㏶', '㏷', '㏸', '㏹', '㏺', '㏻', '㏼', '㏽', '㏾', '㍘', '㍙', '㍚', '㍛', '㍜', '㍝', '㍞', '㍟', '㍠', '㍡', '㍢', '㍣', '㍤', '㍥', '㍦', '㍧', '㍨', '㍩', '㍪', '㍫', '㍬', '㍭', '㍮', '㍯', '㍰']
    },
    {
        category: '表情符號',
        symbols: ['☺', '☻', '☹', '〠', 'ت', 'ヅ', 'ツ', 'ッ', 'シ', 'Ü', 'ϡ', 'ﭢ', '웃', '유', '⍡', '⍢', '⍤', '⍥', '⍨', '⍩', 'ⶆ', '冏', '囧']
    },
    {
        category: '音樂符號',
        symbols: ['𝄞', '♫', '♬', '♪', '♩', '♭', '♪', '♮', '♯', 'ø', 'ヰ', '∮', '𝄡', '𝄢', '𝄪', '𝄫', '¶', '×', '⌒', '∠', '‖', '||:', ':||', '𓏢']
    },
    {
        category: '撲克牌',
        symbols: ['♤', '♡', '♧', '♢', '♠', '♥', '♣', '♦', '🂠', '🂡', '🂢', '🂣', '🂤', '🂥', '🂦', '🂧', '🂨', '🂩', '🂪', '🂫', '🂬', '🂭', '🂮', '🂱', '🂲', '🂳', '🂴', '🂵', '🂶', '🂷', '🂸', '🂹', '🂺', '🂻', '🂼', '🂽', '🂾', '🃁', '🃂', '🃃', '🃄', '🃅', '🃆', '🃇', '🃈', '🃉', '🃊', '🃋', '🃌', '🃍', '🃎', '🃑', '🃒', '🃓', '🃔', '🃕', '🃖', '🃗', '🃘', '🃙', '🃚', '🃛', '🃜', '🃝', '🃞', '🃟']
    },
    {
        category: '骰子',
        symbols: ['⚀', '⚁', '⚂', '⚃', '⚄', '⚅']
    },
    {
        category: '標點符號',
        symbols: ['；', '﹔', '︰', '﹕', '：', '，', '﹐', '、', '．', '﹒', '˙', '·', '‧', '•', '¨', '‥', '…', '。', '？', '﹖', '⁇', '⁉', '⁈', '¿', '‽', '⸘', '؟', '¡', '‼', '！', '❢', '～', '〝', '〞', '′', '‵', '″', '″', '‴', '⁗', '‶', '‷', '〟', '〃', '❛', '❜', '❝', '❞']
    },
    {
        category: '括號符號',
        symbols: ['『', '』', '﹃', '﹄', '「', '」', '﹁', '﹂', '〈', '〉', '《', '》', '【', '】', '〖', '〗', '︻', '︼', '〔', '〕', '︵', '︶', '︷', '︸', '︹', '︺', '︽', '︾', '︿', '﹀', '﹙', '﹚', '﹛', '﹜', '﹝', '﹞', '（', '）', '＜', '＞', '｛', '｝', '⦃', '⦄', '⦑', '⦒', '⦓', '⦔', '⦕', '⦖', '⁅', '⁆', '⸦', '⸧', '⎰', '⎱', '⦇', '⦈', '⊏', '⊐', '⊑', '⊒', '⟦', '⟧', '⸨', '⸩']
    },
    {
        category: '中文大寫',
        symbols: ['壹', '貳', '參', '叁', '肆', '伍', '陸', '柒', '捌', '玖', '拾', '佰', '仟', '萬', '億', '兆', '圓', '角', '分', '零', '整']
    },
    {
        category: '圓圈文字',
        symbols: ['㊬', '㊯', '㊐', '㊊', '㊎', '㊍', '㊌', '㊋', '㊏', '㊣', '㊚', '㊛', '㊙', '㊟', '㊕', '㊗', '㊡', '㊝', '㊭', '㊮', '㊑', '㊒', '㊓', '㊔', '㊕', '㊖', '㊘', '㊜', '㊞', '㊠', '㊢', '㊤', '㊥', '㊦', '㊧', '㊨', '㊩', '㊪', '㊫', '㊰', '㈰', '㈪', '㉃', '㈲', '㈺', '㉂', '㈾', '㈸', '㈴', '㈻', '㈮', '㈭', '㈬', '㈫', '㈯', '㈷', '㈼', '㈳', '㈽', '㈵', '㈶', '㈿', '㈹', '㈱', '㋐', '㋑', '㋒', '㋓', '㋔', '㋩', '㋪', '㋫', '㋬', '㋭', '㋕', '㋖', '㋗', '㋘', '㋙', '㋮', '㋯', '㋰', '㋱', '㋲', '㋚', '㋛', '㋜', '㋝', '㋞', '㋳', '㋴', '㋵', '㋟', '㋠', '㋡', '㋢', '㋣', '㋶', '㋷', '㋸', '㋹', '㋺', '㋤', '㋥', '㋦', '㋧', '㋨', '㋻', '㋼', '㋽', '㋾', '㉠', '㉡', '㉢', '㉣', '㉤', '㉥', '㉦', '㉧', '㉨', '㉩', '㉪', '㉫', '㉬', '㉭', '㉮', '㉯', '㉰', '㉱', '㉲', '㉳', '㉴', '㉵', '㉶', '㉷', '㉸', '㉹', '㉺', '㉻', '㈀', '㈁', '㈂', '㈃', '㈄', '㈅', '㈆', '㈇', '㈈', '㈉', '㈊', '㈋', '㈌', '㈍', '㈎', '㈏', '㈐', '㈑', '㈒', '㈓', '㈔', '㈕', '㈖', '㈗', '㈘', '㈙', '㈚', '㈛', '㈜']
    },
    {
        category: '集錦文字',
        symbols: ['🅊', '🅋', '🅌', '🅍', '🅎', '🅏', '🆊', '🆋', '🆌', '🆍', '🆏', '🈂', '🈓', '🈩', '🈔', '🈪', '🈬', '🈭', '🈮', '🈜', '🈝', '🈐', '🈑', '🈒', '🈕', '🈖', '🈗', '🈘', '🈙', '🈛', '🈞', '🈟', '🈠', '🈡', '🈢', '🈣', '🈤', '🈥', '🈦', '🈧', '🈨', '🈫', '🈰', '🈱', '🈷']
    },
    {
        category: '盲文（點字）',
        symbols: [
            '⠁','⠂','⠃','⠄','⠅','⠆','⠇','⠈','⠉','⠊','⠋','⠌','⠍','⠎','⠏','⠐','⠑','⠒','⠓','⠔','⠕','⠖','⠗','⠘','⠙','⠚','⠛','⠜','⠝','⠞','⠟',
            '⠠','⠡','⠢','⠣','⠤','⠥','⠦','⠧','⠨','⠩','⠪','⠫','⠬','⠭','⠮','⠯','⠰','⠱','⠲','⠳','⠴','⠵','⠶','⠷','⠸','⠹','⠺','⠻','⠼','⠽','⠾','⠿',
            '⡀','⡁','⡂','⡃','⡄','⡅','⡆','⡇','⡈','⡉','⡊','⡋','⡌','⡍','⡎','⡏','⡐','⡑','⡒','⡓','⡔','⡕','⡖','⡗','⡘','⡙','⡚','⡛','⡜','⡝','⡞','⡟',
            '⡠','⡡','⡢','⡣','⡤','⡥','⡦','⡧','⡨','⡩','⡪','⡫','⡬','⡭','⡮','⡯','⡰','⡱','⡲','⡳','⡴','⡵','⡶','⡷','⡸','⡹','⡺','⡻','⡼','⡽','⡾','⡿',
            '⢀','⢁','⢂','⢃','⢄','⢅','⢆','⢇','⢈','⢉','⢊','⢋','⢌','⢍','⢎','⢏','⢐','⢑','⢒','⢓','⢔','⢕','⢖','⢗','⢘','⢙','⢚','⢛','⢜','⢝','⢞','⢟',
            '⢠','⢡','⢢','⢣','⢤','⢥','⢦','⢧','⢨','⢩','⢪','⢫','⢬','⢭','⢮','⢯','⢰','⢱','⢲','⢳','⢴','⢵','⢶','⢷','⢸','⢹','⢺','⢻','⢼','⢽','⢾','⢿',
            '⣀','⣁','⣂','⣃','⣄','⣅','⣆','⣇','⣈','⣉','⣊','⣋','⣌','⣍','⣎','⣏','⣐','⣑','⣒','⣓','⣔','⣕','⣖','⣗','⣘','⣙','⣚','⣛','⣜','⣝','⣞','⣟',
            '⣠','⣡','⣢','⣣','⣤','⣥','⣦','⣧','⣨','⣩','⣪','⣫','⣬','⣭','⣮','⣯','⣰','⣱','⣲','⣳','⣴','⣵','⣶','⣷','⣸','⣹','⣺','⣻','⣼','⣽','⣾','⣿'
        ]
    },
    {
        category: '希臘字母',
        symbols: [
            'Φ','Χ','Ψ','Ω','α','ν','ξ','ο','π','ρ','σ','τ','υ','φ','χ','ψ','ω','Α','Β','Γ','Δ','Ε','Ζ','Η','Θ','Ι','Κ','Λ','Μ','Ν','Ξ','Ο','Π','Ρ','Σ','Τ','Υ','ζ','η','θ','λ','μ','δ','β'
        ]
    },
    {
        category: '日文符號',
        symbols: [
            'ア','ナ','ス','イ','シ','リ','ー','ズ','か','ら','ド','ム','を','プ','レ','ゼ','ン','り','や','か','な','ベ','ル','ガ','モ','ッ','ト','オ','ジ','ビ','タ','か','ら','ロ','ピ','カ','な','フ','ア','や','ャ','ク','ヤ','な','ど','が','ミ','ド','っ','ぱ','く','ム','バ','ニ','ラ','サ','ダ','ウ','の','ブ','は','と','て','も','お','い','し','そ','う','な','り','ん','か','マ','カ','ョ','パ','す','ゅ','ま','゚','゜','ハ','ヷ','ヸ','ヹ','ヺ','ホ','ボ','ポ','ゝ','ゞ','ヘ','ペ','ぐ','゛','゠','・','㍐','㋿','㍻','㍼','㍽','㍾','㍿'
        ]
    },
    {
        category: '韓文符號',
        symbols: [
            'ㄱ','ㄲ','ㄳ','ㄴ','ㄵ','ㄶ','ㄷ','ㄸ','ㄹ','ㄺ','ㄻ','ㄼ','ㄽ','ㄾ','ㄿ','ㅀ','ㅁ','ㅂ','ㅃ','ㅄ','ㅅ','ㅆ','ㅇ','ㅈ','ㅉ','ㅊ','ㅋ','ㅌ','ㅍ','ㅎ','ㅏ','ㅐ','ㅑ','ㅒ','ㅓ','ㅔ','ㅕ','ㅖ','ㅗ','ㅘ','ㅙ','ㅚ','ㅛ','ㅜ','ㅝ','ㅞ','ㅟ','ㅠ','ㅡ','ㅢ','ㅥ','ㅦ','ㅧ','ㅨ','ㅩ','ㅪ','ㅫ','ㅬ','ㅭ','ㅮ','ㅯ','ㅰ','ㅱ','ㅲ','ㅳ','ㅴ','ㅵ','ㅶ','ㅷ','ㅸ','ㅹ','ㅺ','ㅻ','ㅼ','ㅽ','ㅾ','ㅿ','ㆀ','ㆁ','ㆂ','ㆃ','ㆄ','ㆅ','ㆆ','ㆇ','ㆈ','ㆉ','ㆊ','ー','㉿','〮','〯'
        ]
    },
    {
        category: '特殊字母符號',
        symbols: [
            'Ä','Ã','Ç','Ê','Ë','Î','Ï','Ð','Ñ','Õ','Ö','Û','Ü','ã','ä','ê','ë','î','ï','õ','ö','Ÿ','Ŵ','Ž','Ť','Ř','Ň','ĩ','ħ','ą','č','Ĥ'
        ]
    },
    {
        category: '英文音標',
        symbols: [
            'i:','ə:','a:','ɔ:','u:','i','e','æ','ə','ʌ','ɔ','u','ei','ai','ɔi','əu','au','iə','εə','uə','p','t','k','f','θ','s','ʃ','h','tʃ','ts','tr','b','d','g','v','ð','z','ʒ','r','dʒ','dz','dr','j','w','m','n','ŋ','l'
        ]
    },
    {
        category: '拼音',
        symbols: [
            'Ā','Á','Ǎ','À','Ē','É','Ě','È','Ī','Í','Ǐ','Ì','Ō','Ó','Ǒ','Ò','Ū','Ú','Ǔ','Ù','Ǖ','Ǘ','Ǚ','Ǜ','Ü','ā','á','ǎ','à','ä','ē','é','ě','è','ê','ī','í','ǐ','ì','ō','ó','ǒ','ò','ū','ú','ǔ','ǔ','ǖ','ǘ','ǚ','ǜ','ü','ń','ň','ǹ','ḿ','m̀'
        ]
    },
    {
        category: '注音',
        symbols: [
            'ㄅ','ㄆ','ㄇ','ㄈ','ㄉ','ㄊ','ㄋ','ㄌ','ㄍ','ㄎ','ㄏ','ㄐ','ㄑ','ㄒ','ㄓ','ㄔ','ㄕ','ㄖ','ㄗ','ㄘ','ㄙ','ㄚ','ㄛ','ㄜ','ㄝ','ㄞ','ㄟ','ㄠ','ㄡ','ㄢ','ㄣ','ㄤ','ㄥ','ㄦ','ㄧ','ㄨ','ㄩ','˙','ˊ','ˇ','ˋ'
        ]
    },
    {
        category: '國際音標',
        symbols: [
            'ɐ','ɑ','ɒ','ɓ','ɔ','ɕ','ɖ','ɗ','ɘ','ə','ɚ','ɛ','ɜ','ɝ','ɞ','ɟ','ɠ','ɡ','ɢ','ɣ','ɤ','ɥ','ɦ','ɧ','ɨ','ɩ','ɪ','ɫ','ɬ','ɭ','ɮ','ɯ','ɰ','ɱ','ɲ','ɳ','ɴ','ɴ','ɶ','ɷ','ɸ','ɹ','ɺ','ɻ','ɼ','ɽ','ɾ','ɿ','ʀ','ʁ','ʂ','ʃ','ʄ','ʅ','ʆ','ʇ','ʈ','ʉ','ʊ','ʋ','ʌ','ʍ','ʎ','ʏ','ʐ','ʑ','ʒ','ʓ','ʔ','ʕ','ʖ','ʗ','ʘ','ʙ','ʚ','ʛ','ʜ','ʝ','ʞ','ʟ','ʠ','ʡ','ʢ','ʣ','ʤ','ʥ','ʦ','ʧ','ʨ','ʩ','ʪ','ʫ','ʬ','ʭ'
        ]
    },
    {
        category: '藏文符號',
        symbols: [
            'ཉ','ེ','ར','ཏ','ཡ','ུ','ི','ོ','པ','ལ','ཀ','ཇ','ཧ','ག','ང','ད','ས','འ','ཟ','ཛ','ཅ','ཙ','བ','ན','མ','་','ྃ','ཾ','྆','྆','྇','ྈ','ྉ','ྊ','྅','ྋ','ཎ','བ','ཚ','ཆ','ཥ','ཞླ','ཁ','ཇ','ཿ','ཽ','ྀ','ྭ','ྱ','ྲ','ཻ','ཝ','ཨ','༅','༄','࿓','࿔','༇','࿑','༈','ༀ','༉','༊','࿐','༂','༃','༎','།','༏','༐','༑','༒','༓','༔','༵','༷','༴','༶','༜','࿏','࿄','࿆','࿇','࿈','࿂','༕','༖','༗','༿','༾','༘','༙','༼','༽','༺','༻','༡','༢','༣','༤','༥','༦','༧','༨','༩','༠'
        ]
    },
    {
        category: '植物符號',
        symbols: [
            '𓁋','𓁙','𓆸','𓆹','𓆼','𓇊','𓇚','𓇕','𓇗','𓋇','𓆭','𓆰','𓆱','𓇋','𓇑','𓇛','𓇟','𓇣','❀','✿','❁','❃','❋','✽','✾','𒂭','⚜','ꕤ','ꕥ','𓇬','⚘','֍','֎','𑁍','𓄟','𓍯','𓋼','𓍊','𐌙','𐊵','𐡘','𐡙','𐡚'
        ]
    },
    {
        category: '人物符號',
        symbols: [
            '𓀞','𓀟','𓀠','𓀡','𓀢','𓀣','𓀤','𓀥','𓀦','𓀧','𓀨','𓀩','𓀪','𓀫','𓀬','𓀀','𓀁','𓀂','𓀃','𓀄','𓀅','𓀆','𓀇','𓀈','𓀉','𓀊','𓀋','𓀌','𓀍','𓀎','𓀏','𓀑','𓀒','𓀓','𓀔','𓀕','𓀖','𓀗','𓀘','𓀙','𓀚','𓀛','𓀜','𓀝','𓁁','𓁂','𓁃','𓁄','𓁅','𓁆','𓁇','𓁈','𓁉','𓁊','𓁋','𓁌','𓁍','𓁎','𓁏','𓁐','𓁑','𓁒','𓁓','𓁔','𓁕','𓁖','𓁗','𓁘','𓁙','𓀭','𓀮','𓀯','𓀰','𓀱','𓀲','𓀳','𓀴','𓀵','𓀶','𓀷','𓀸','𓀹','𓀺','𓀻','𓀼','𓀽','𓀾','𓀿','𓁀','𓁲','𓁚','𓁛','𓁜','𓁝','𓁞','𓁟','𓁠','𓁡','𓁢','𓁣','𓁤','𓁥','𓁦','𓁧','𓁨','𓁩','𓁪','𓁫','𓁬','𓁭','𓁮','𓁯','𓁰','𓁱','𓁳','𓁴','𓁵'
        ]
    },
    {
        category: '動物符號',
        symbols: [
            '𖠊','𖢥','𖠌','𖠍','𖠐','𖠅','ଲ','ꙮ','𖠽','𖡉','𖢘','𖠑','𖠒','𖡒','𖢂','𖤖','𖠓','𓃥','𓃠','𓃰','𓃱','𓃯','𓃭','𓃸','𓃵','𓃗','𓃘','𓃙','𓃟','𓄀','𓄁','𓄂','𓄃','𓃚','𓃛','𓃜','𓃝','𓃞','𓃒','𓃓','𓃔','𓃕','𓃖','𓃡','𓃢','𓃦','𓃩','𓃫','𓃬','𓃮','𓃲','𓃴','𓃶','𓃷','𓃹','𓃻','𓃽','𓃾','𓃿','𓄄','𓄅','𓄆','𓄇','𓆇','𓆈','𓆉','𓆌','𓆏','𓆗','𓆘','𓆙','𓆚','𓆐','𓆑','𓆒','𓆓','𓆔','𓆕','𓆖','𓆊','𓆍','𓆣','𓆤','𓆥','𓆦','𓆧','𓆨','𓆛','𓆜','𓆝','𓆞','𓆟','𓆠','𓆡','𓆢','𓄿','𓅀','𓅁','𓅂','𓅃','𓅄','𓅅','𓅆','𓅇','𓅈','𓅉','𓅊','𓅋','𓅌','𓅍','𓅎','𓅏','𓅐','𓅑','𓅒','𓅓','𓅔','𓅕','𓅖','𓅗','𓅘','𓅙','𓅚','𓅛','𓅜','𓅝','𓅞','𓅟','𓅠','𓅡','𓅢','𓅣','𓅤','𓅥','𓅦','𓅧','𓅨','𓅩','𓅪','𓅫','𓅬','𓅭','𓅮','𓅯','𓅰','𓅱','𓅲','𓅳','𓅴','𓅵','𓅶','𓅷','𓅸','𓅹','𓅺','𓅻','𓅼','𓅽','𓅾','𓅿','𓆀','𓆁','𓆂','𓆃','𓆆'
        ]
    }
];

const SymbolClient: React.FC<SymbolClientProps> = ({ lng }) => {
    const clipboard = useClipboard({ timeout: 1500 });
    const [copiedSymbol, setCopiedSymbol] = useState<string | null>(null);
    const { t } = useTranslation(lng, 'common');

    const handleCopy = (symbol: string) => {
        clipboard.copy(symbol);
        setCopiedSymbol(symbol);
    };

    return (
        <Container size="xs" mt="lg">
            <Title order={3} ta="center">特殊符號複製工具</Title>
            {symbolCategories.map((category) => (
                <div key={category.category}>
                    <Title order={4} mt="md" mb="sm">{category.category}</Title>
                    <VirtuosoGrid
                        style={{ height: '300px' }}
                        totalCount={category.symbols.length}
                        listClassName="symbol-grid"
                        itemContent={(index) => {
                            const symbol = category.symbols[index];
                            return (
                                <Tooltip
                                    label={copiedSymbol === symbol ? t('copied') : t('copy')}
                                    withArrow
                                    position="top"
                                >
                                    <Paper
                                        shadow="xs"
                                        p="xs"
                                        radius="sm"
                                        ta="center"
                                        style={{ cursor: 'pointer', fontSize: '24px' }}
                                        onClick={() => handleCopy(symbol)}
                                    >
                                        {symbol}
                                    </Paper>
                                </Tooltip>
                            );
                        }}
                    />
                    <style jsx global>{`
                        .symbol-grid {
                            display: grid;
                            grid-template-columns: repeat(4, 1fr);
                            gap: 8px;
                            padding: 8px;
                        }
                        .symbol-grid > div {
                            width: 100% !important;
                            margin: 0 !important;
                        }
                    `}</style>
                </div>
            ))}
        </Container>
    );
};

export default SymbolClient;
